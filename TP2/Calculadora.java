...
class Calculadora {
	...
	Action[] actions = {
		...
		new Action(
			...
			new Function() { public void call() {
				...
				// Se asegura que min sea siempre el valor más chico
				// y max sea siempre el valor más grande.
				if (min > max) {
					double temp = min;
					min = max;
					max = temp;
				...
				// Usé Math.random en vez de la clase Random
				// porque lo veía más conveniente.
	...
	// Muestra las posibles acciones en la terminal.
	void mainScreen() {
		while (true) {
			clear();
			System.out.println("Seleccione la acción.");
			
			// Itera la lista de acciones para mostrarlas.
			...
			// Pide el índice de una acción al usuario
			// indefinidamente hasta que obtener uno válido.
			while (true) {
			...
			// Salir del programa si se elige la opción cero.
			if (actionIndex == 0) {
	...
	// Limpia la terminal independientemente del OS.
	void clear() {
		try {
